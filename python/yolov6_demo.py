...
def inference(model, img, precision, backend, thread):
    ...
    det_boxes, det_classes, det_scores, num_dets = net.onForward(input_var)
    det_boxes = MNN.expr.convert(det_boxes, MNN.expr.NCHW)
    box_num = num_dets.read_as_tuple()[0]
    for i in range(box_num):
        x0, y0, x1, y1 = det_boxes[0, i].read_as_tuple()
        y0 = int(y0 * scale)
        y1 = int(y1 * scale)
        x0 = int(x0 * scale)
        x1 = int(x1 * scale)
        class_idx = det_classes.read_as_tuple()[i]
        score = det_scores.read_as_tuple()[i]
